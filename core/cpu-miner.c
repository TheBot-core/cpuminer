...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		float khash_total = 1e-3 * hashrate;

		float khash_thread_avg = khash_total / opt_n_threads;
		...
		sprintf(buffer, "{\n"
						...
						"	\"khash_total\": %f,\n"
						"	\"khash_thread_avg\": %f\n"
						"}", opt_n_threads, num_processors, rpc_url, rpc_user, algo_name, khash_total, khash_thread_avg);
		...
		fwrite(buffer, strlen(buffer), sizeof(char), fptr);
		fclose(fptr);

		//applog(LOG_INFO, "Writing data.json");
